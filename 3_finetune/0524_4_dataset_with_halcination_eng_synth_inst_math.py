...
data_folder="data/0524with_halcination_little_codes_synth_eng_math"
...
# %%
# meta math
#数学的な基礎力が低く、勉強してもあまり意味がなさそう:  加えて､gpt-4で生成している模様なので使わない
"""
...
for original_record in iter(ds):
    ...
    records.append(text)
    # 2 epoch分学習させる
    text=f"{question_template}次の計算をしなさい｡\n{q}{answer_template}{a}"
    records.append(text)
...
for original_record in iter(ds):
    ...
    text=f"{question_template}{q}{answer_template}{a}"
    records.append(text)
code_ds_dict["bigcode/self-oss-instruct-sc2-exec-filter-50k"]=records
print("big code done")
...
code_ds_dict["sansu_M"]=calc_problems
